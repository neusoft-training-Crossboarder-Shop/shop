...
import neu.train.project.mvo.service.impl.MvoCommonService;
...
public class OrderController extends BaseController {
    ...
    @PreAuthorize("@ss.hasAnyPermi('bvo:order:list,mvo:order:list')")
    ...
    @PreAuthorize("@ss.hasPermi('bvo:order:list')")
    @Transactional
    ...
    @ApiOperation(value = "查询收货地址一个，给BVO,MVO用的",httpMethod = "GET",notes = "必选参数：原始订单Id stoId")
    @PreAuthorize("@ss.hasAnyPermi('bvo:order:list,mvo:order:list')")
    ...
    @PreAuthorize("@ss.hasAnyPermi('bvo:order:list,mvo:order:list')")
    @GetMapping("bvo/order/salOrder/{stoId}")
    public AjaxResult getSaoByStoId(@PathVariable("stoId") int stoId){
    ...
    public TableDataInfo listSao(GetASaoQueryByTime getASaoQueryByTime){
            getASaoQueryByTime.setMvoId(mvoCommonService.getManId());
            startPage();
